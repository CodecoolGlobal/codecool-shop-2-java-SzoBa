...
public class ProductController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        CountryDao productCategoryDataStore = CountryDaoMem.getInstance();

        TemplateEngine engine = TemplateEngineUtil.getTemplateEngine(req.getServletContext());
        WebContext context = new WebContext(req, resp, req.getServletContext());

        context.setVariable("country", productCategoryDataStore.find(1));
        context.setVariable("matchDetails", productDataStore.getBy(productCategoryDataStore.find(1)));
